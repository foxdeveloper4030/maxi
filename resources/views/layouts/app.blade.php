...
<head>
    ...
    <title>@yield('title') ||  {{\App\PublicModel::EnappName()}}</title>
    ...
    <link href="{{asset('public/assets/css/new.css')}}" rel="stylesheet"/>
    ...
    {{--{!!--}}
    {{--htmlScriptTagJsApiV3(['action' => route('login')])--}}
    {{--!!}--}}
</head>

<body class="index-page sidebar-collapse"
      onload="@if(session('status'))
              statusLoadFunc();
      @endif
              ">
                ...
                <div class="row banner-ads ">
                    <div class="col-12">
                        <section class="banner ">
                            <a class="widget-suggestion widget card" href="{{\App\Banner::query()->find(1)->link}}">
                                <img src="{{url('')}}/{{\App\Banner::query()->find(1)->url}}"
                                     alt="{{\App\Banner::query()->find(1)->title}}">
                            </a>
                        </section>
                    </div>
                </div>
...
<script>
    ...
    function loadproduct(slug) {
        loader_show();
        $(document).ready(function () {
            $.get("{{route('main.product.show.load')}}/" + slug, function (data, status) {
                loader_dismis();
