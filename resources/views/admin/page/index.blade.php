                        ...
                        <table class="table table-hover">
                            ...
                            @foreach($pages as $page)
                                <tr style="text-align: center">
                                    <td>{{$counter}}</td>
                                    <td>{{$page->title}}</td>
                                    <td>{{\Illuminate\Support\Str::limit($page->body,90)}}</td>
                                    <td style="color: #001900;font-weight: bold;">{{\Illuminate\Support\Str::limit($page->locationName,20)}}</td>
                                    @if ($page->imgUrl)
                                        <td data-toggle="modal" data-target="#pageeditModal{{$page->id}}"
                                            style="cursor: pointer;color: #3c8dbc">عکس
                                        </td>
                                        <!-- Modal -->
                                        <div id="pageeditModal{{$page->id}}" class="modal fade" role="dialog">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="form-account-row">
                                                            ...
                                                            transform: translate(-1%,-7%);z-index: 0!important;"
                                                                 src="{{asset($page->imgUrl)}}" alt="{{$page->title}}">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{route('pages.pageCreate',$page->title)}}" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>
                                    </td>
                                </tr>
                                @php($counter++)
                            @endforeach
