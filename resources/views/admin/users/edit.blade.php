                ...
                <div class="col-md-3">
                    ...
                    <div class="box box-primary">
                        <div class="box-body box-profile">
                            ...
                            ?>
                            <a href="{{route('admin.users.showuser',['user'=>$user->id])}}?active=1" class="btn btn-primary btn-block"><b>
                                    @if($user->active==1)
                                        غیر فعال کردن
                                    @else
                                        فعال کردن
                        ...
                        <div class="tab-content">
                            <div class="active tab-pane" id="activity">
                                <table class="table table-hover">
                                    ...
                                    @forelse(\App\Order::query()->where('id_user','=',$user->id)->get() as $order)
                                        <tr>
                                            <td>{{$order->id}}</td>
                                            <td>{{\App\User::query()->find($order->id_user)->firstname}}.{{(new \App\PublicModel())->short_string(\App\User::query()->find($order->id_user)->lastname)}}</td>
                                            <td>{{$order->price}}</td>
                                            <td>
                                                {!! (new \App\OrderModul($order))->state() !!}
                                            </td>
                                            <td>
                                                <a href="{{route('admin.order.show',['id'=>$order->id])}}" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>
                                            </td>
                                        </tr>
                                    @empty
